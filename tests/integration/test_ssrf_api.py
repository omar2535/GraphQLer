...
Detectors exercised:
  - SSRF Injection (fetchUrl / pingEndpoint / registerWebhook — raw outbound
    HTTP requests whose connection-error messages are reflected back verbatim)
"""
...
class TestSSRFAPI(GraphQLerIntegrationTestCase):
    ...
    def test_ssrf_injection_detected(self):
        ...
        self.assertTrue(
            is_detection_flagged(vulns, "SSRF Injection"),
            f"Expected SSRF to be flagged. Got: {vulns}",
        )
