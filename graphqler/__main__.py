...
from graphqler.utils.cli_utils import set_auth_token_constant, set_idor_auth_token_constant
...
def run_compile_chains_mode(compiler: Compiler, path: str, url: str):
    ...
    manifest = validate_manifest(path, "graph", compiler.settings, expected_endpoint=url or None)
    compiler.url = url or manifest.get("endpoint")
...
def run_fuzz_mode(fuzzer: Fuzzer, path: str, url: str):
    ...
    stats = fuzzer.stats
    stats.set_file_paths(path, reset=not fuzzer.settings.RESUME)
...
def main(args: dict):
    ...
    if args.get("authorization_differential"):
        config.AUTHORIZATION_DIFFERENTIAL = True
        print("(P) Multi-profile authorization differential testing enabled")
    if args.get("resume"):
        if args["mode"] != "fuzz":
            raise SystemExit("--resume is only valid with fuzz mode")
        config.RESUME = True
        print("(P) Resuming from the latest run checkpoint")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--use-llm",
        help="enable LLM-powered features: dependency graph inference, endpoint classification, IDOR chain classification, and UAF chain classification (requires LLM_MODEL and credentials)",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "--no-llm-compilation",
        help="disable LLM during the compilation phase (dependency resolver, IDOR/UAF chain classifiers) — overrides --use-llm for that phase",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "--no-llm-fuzzing",
        help="disable LLM during the fuzzing phase (payload generation, error retry, endpoint classification, report) — overrides --use-llm for that phase",
        action="store_true",
        default=False,
    )
    ...
    parser.add_argument(
        "--no-dependency-graph", help="ablation: disable dependency-graph chain ordering — all nodes run independently without chaining", action="store_true", default=False
    )
    ...
    parser.add_argument(
        "--subscriptions",
        help="enable fuzzing of GraphQL subscriptions via WebSocket (disabled by default — requires WebSocket support on the target)",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "--authorization-differential",
        help="replay likely private queries, mutations, and subscriptions under anonymous and alternate --auth profiles",
        action="store_true",
        default=False,
    )
    parser.add_argument("--resume", help="resume an interrupted fuzz run from its latest checkpoint", action="store_true", default=False)
    ...
    parser.add_argument(
        "--mcp-transport",
        help="MCP transport to use: 'stdio' (default), 'sse', 'streamable-http', or 'http'",
        default="stdio",
